...
from models import VisionIndex,VisionSWIndustry
import config
...
class StockPool(object):
    ...
    def fetch_index_db(self, trade_date_list, index):
        query = select([VisionIndex.trade_date, VisionIndex.symbol]).where(
            and_(
                VisionIndex.trade_date.in_(trade_date_list),
                VisionIndex.isymbol==index
            )
        ).order_by(VisionIndex.trade_date)
        df = pd.read_sql(query, con=self._conn)
        df.sort_values(by='trade_date')
        return df
        
    def fetch_sw_industry_db(self, trade_date_list, industry_set):
        query = select([VisionSWIndustry.trade_date, VisionSWIndustry.symbol,
                        VisionSWIndustry.iname,
                       VisionSWIndustry.isymbol]).where(
            and_(
                VisionSWIndustry.trade_date.in_(trade_date_list),
                VisionSWIndustry.isymbol.in_(industry_set)
            )
        ).order_by(VisionSWIndustry.trade_date)
        df = pd.read_sql(query, con=self._conn)
        df.sort_values(by='trade_date')
        return df
